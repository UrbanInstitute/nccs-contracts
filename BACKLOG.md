...
|---|------|-------|----------------|
| Z1 | **Rebuild + re-publish everything the ZIP defect touched** | `nccs-data-bmf` | **Blocks the rest of Z.** `.clean_zip()` dropped the ZIP for legacy rows in ME/NH/VT/MA/RI/CT/NJ/PR/VI (leading zeros stripped upstream, `^\d{5}` returned NA). Fix + `assert_zip_integrity()` gate sit on branch `fix/legacy-zip-leading-zeros`, unpushed, needing an ADR since it changes published column values. Sequence: push + ADR, re-run 55 legacy vintages, rebuild Unified BMF + state marts, re-geocode the ~124k ZIP-less addresses (delta, not a full cycle), rebuild + re-publish the address log, re-run `validate_address_crosswalk.R` (2 checks currently fail). |

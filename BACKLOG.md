...
| Z16 | **Fix the NTEEV2 x00 rule + pre-0032 stale vintages — ADR 0048 (Executing)** | `nccs-data-bmf` then `nccs` | ADR 0048 Executing on `fix/0048-nteev2-x00`. `nteev2_code` double-encodes specialty/common codes 01-19 (`B11` -> `EDU-B11-MS`; spec + our own legacy lookup say `EDU-B00-MS`): 313,662 rows (8.48%) of Unified BMF 2026_08. Round-1 review's reconciliation check found a SECOND defect (Decision #3a): 177,374 rows stale from the ~37 pre-0032 current-monthly vintages L1 never reprocessed. Fix in the single derivation path + first test suite + NA-safe invariants + `scripts/check_nteev2_reconciliation.R`. Reprocess scope = 85 legacy + ~37 current-monthly + current, then unified/geocoded/marts/ntee-resolved via Z13 publisher. Pilot task for the Claude-implements/Codex-reviews workflow. Found by NODC (`matchdb` `faa4fe8`), credited in the DRAFT release note. |

...
| Z27 | ~~Empty-EIN quality check: threshold instead of any-row failure~~ | *closed* | **DONE 2026-09-21** (nccs-data-bmf #59 merged). Decision as proposed: a critical field fails the report only when more than 1 in 10,000 rows is empty (`CRITICAL_FIELD_MAX_MISSING_SHARE <- 1e-4`); the count is always recorded (`critical_field_missing`), printed and shown in the HTML report; the Z9 emptied-column check is unchanged. Review addition: the allowance covers blank source rows only; the output may never have more empty EINs than the source had (pre-check `null_counts`), or the report fails whatever the share. Verified on the published June 1996 legacy file: 21 empty of 1,087,620 rows (0.0019%), report passes, so that month no longer needs the gate override. Background: June 1996 legacy has 21 rows with no EIN and the hard gate (Z9) stopped the month for that; the rows carry the EIN-missing flag and cannot join to anything. |

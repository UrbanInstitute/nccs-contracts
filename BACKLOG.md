...
| Z9 | ~~Wire `report$passed` into a hard gate~~ | `nccs-data-bmf` | **DONE 2026-09-16** (nccs-data-bmf PR #53). Both the monthly and the legacy runner now stop before any upload when the quality report fails; `STRICT_QUALITY_GATES <- FALSE` is the deliberate override. The report itself was too weak to have caught the July ZIP defect (it only checked row counts and empty EINs), so it gained a new check: a column that is completely empty after transformation while its source column had values fails the report. Columns that were already empty in the source (common in old legacy months) are not flagged. Dry run: of the 121 published months, 120 pass the existing checks and one fails (June 1996 legacy: 21 rows with an empty EIN). The new emptied-column check could not be replayed on old reports because they carry no source counts; it is covered by unit tests and a local end-to-end run of the July 2026 file. **Open decision:** what to do with the 21 empty-EIN rows in June 1996 if that month is ever rebuilt (drop the rows, or run with the override). |

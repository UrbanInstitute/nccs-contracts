...
| G2 | **ADR-NEEDED:** extend the Unified BMF rename to its geocoded extension (`bmf-master-geocoded` → geocoded Unified BMF) | Next free ADR number is **0039**. Same non-silent-supersession pattern as ADR 0037, one layer deeper: stand up a new geocoded path (e.g. `geocoding/unified-bmf/`) alongside the current `geocoding/bmf-master/`, migrate `nccsdata::nccs_read()` (`R/nccs_read.R:401`) + `sector-in-brief-api`'s hardcoded reads over a 90-day window, then archive the old path. Bigger blast radius than the un-geocoded rename — `geocoding/bmf-master/merged/...` is the literal canonical `nccs_read()` surface — so this gets its own deliberate ADR, not a rider on 0037. Raised 2026-07-01 while checking whether the Unified BMF needs a fresh geocoded companion (see G1). |
...
| G1 | Fix broken geocoding input path + refresh the geocoded master | `nccs-data-bmf` | **Found 2026-07-01. Sequenced after L1.** `run_master_geocoding.R`'s `MASTER_PARQUET_PATH` still hardcodes `data/master/bmf_master.parquet`, but `run_master_pipeline.R` now writes `data/master/bmf_unified.parquet` (stem changed to `UNIFIED_STEM` under ADR 0037) — the next geocoding run will fail until this is repointed. Separately, the published geocoded artifact (`s3://nccsdata/geocoding/bmf-master/merged/`) is stale at 2026-06-17 — predates the 07-01 Unified BMF build and L1's legacy-NTEE fix, missing `ein_prefixed`/`EIN2` and the newer vintage data. No ADR needed — this is a currency refresh under the existing `bmf-master-geocoded` name/path, not a rename (see G2 for the separate naming question). |

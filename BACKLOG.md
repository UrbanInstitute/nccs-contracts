...
| Z2 | Regenerate the BMF catalog and drop the ZIP caveat | `nccs` | After Z1. `catalogs/get-aws-files.R bmf`, re-render `catalog-bmf.qmd`, delete the warning callout added in nccs #92, refresh the coverage figure if re-geocoding moves it off 82.7%. **ADR 0048 note:** re-render AFTER the 0048 republish so the catalog picks up corrected nteev2 values in one pass; also swap the e-file catalog pointer to v2_2 here. |
...
| Z14 | ~~Repoint sector-in-brief-api + nccsdata to `geocoding/unified-bmf/latest/`~~ | `sector-in-brief-api` + `nccsdata` | **DONE 2026-09-16** (PRs nccsdata #25, sector-in-brief-api #21; both read `geocoding/unified-bmf/latest/`, contract consumer notes updated). Verified byte-identical objects on both prefixes and anonymous HTTPS access before repointing. Closes G2/G3. Side findings: (a) `sector-in-brief-api` has no `contracts-guard.yml` caller, contrary to ADR 0022; (b) nccsdata cache download of the 641 MB parquet hits the 60 s `download.file` default timeout and falls back to S3 (pre-existing, tracked as Z22). The old `geocoding/bmf-master/` path can archive at its own 2026-09-30 cutover (fresh 90-day clock from the 2026-07-02 geocoded publish per ADR 0039; the 2026-09-28 date is the un-geocoded `master/bmf/` cutover) once these merge. |
...
| Z19 | State-mart CSV stem: finish the ADR 0039 rename (`bmf_master_{ST}.csv` -> `bmf_unified_{ST}.csv`) | `nccs-data-bmf` then `nccs-contracts` | **New 2026-09-15.** Confirmed drift: live `unified/bmf/state_marts/csv/` and the code (`R/master_state_marts.R:87`) write `bmf_master_{ST}.csv`; `contracts/unified-bmf.yml` documents `bmf_unified_{ST}.csv`. Republished unchanged under ADR 0048. Needs a decision (rename with dual-write window, or amend the contract), then one bmf PR. |
| Z20 | Legacy vintage inventory mismatches surfaced by the ADR 0048 reprocess | `nccs-data-bmf` | **New 2026-09-15.** (a) `run_all_legacy.sh` skips 2018-12 by default (sequence-ID EINs) but `processed/bmf-legacy/2018_12/` was published anyway (pre-0041 schema); reprocessed in scope on 2026-09-15 (maintainer call), now schema-consistent, 2,960 duplicate EINs. Decide: drop it from the skip list for good, or from the prefix. (b) `processed/bmf/2024_09/` and `2024_10/` had NO processed CSV (only dictionary + quality report, 2026-01-22) until 2026-09-15; the Unified BMF was silently built from 33 current vintages. Add a completeness gate to the master build (every vintage dir must carry its CSV). |
| Z21 | EC2 batch hardening from the ADR 0048 run | `nccs-data-bmf` | **New 2026-09-15.** One PR: (1) `setup_ec2.sh` installs `aws.ec2metadata` from GitHub (0.2.2; CRAN 0.2.0 cannot do IMDSv2 -> R 403s on IMDSv2-required boxes while the CLI works) and the runners export `USE_IMDS_TOKEN=TRUE`; (2) `exists()`-guard `ENABLE_S3_UPLOAD` in `run_master_pipeline.R`, `run_master_geocoding.R`, `run_master_state_marts.R` (currently unconditional; patched box-locally only); (3) `nccs-bmf-batch` role needs `geocoding-codestar-prod` put/head/get so `retrieve` can run on the box (delta was hand-submitted from the laptop this time); (4) Quarto render of the unified quality report fails on the box (stale HTML on S3 since 2026-08-11); (5) delete the ~100 GB `intermediate/tmp/adr0048_before/` snapshot once this reconcile merges; (6) `stage3`-style publish needs `R/manifest.R` sourced before `publish_bmf_lookups()` (first attempt failed on `manifest_input_repo`). |
| Z22 | nccsdata cache download timeout + sector-in-brief-api contracts-guard caller | `nccsdata` then `sector-in-brief-api` | **New 2026-09-16**, side findings of Z14. (a) `.bmf_master_source()` downloads the 641 MB geocoded parquet with `utils::download.file` under the 60 s default timeout, so on ordinary connections the cache never fills and every `cache = TRUE` read silently falls back to S3 (stale `.part` files accumulate). Raise the timeout for the call (`options(timeout=)` scoped with `on.exit`) or stream via `curl` (already in Suggests). (b) `sector-in-brief-api` has no `contracts-guard.yml` workflow caller, so its publish/read-surface PRs are not checked for an ADR breadcrumb (ADR 0022 says every repo has one). Add the caller. |
...
| G2 | ~~Repoint `nccsdata::nccs_read()` to the new geocoded path~~ | `nccsdata` | **DONE 2026-09-16** via Z14 (nccsdata #25). |
| G3 | ~~Repoint `sector-in-brief-api`'s hardcoded geocoded-BMF read~~ | `sector-in-brief-api` | **DONE 2026-09-16** via Z14 (sector-in-brief-api #21). |

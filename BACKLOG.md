...
| Z30 | ~~Self-service NTEE lookup by EIN on the website (sharded JSON EIN index)~~ | *closed* | **DONE 2026-09-21.** ADR 0050 Reconciled. nccs-contracts #95/#96, nccs-data-bmf #58, nccs #107 merged; index live at `unified/bmf/ein-index/{v2026_09,latest}/`; page live at `/datasets/ntee/lookup/`. Follow-up (API, ADR 0026): search by organization name. |
| Z27 | Empty-EIN quality check: threshold instead of any-row failure | `nccs-data-bmf` | **PR OPEN 2026-09-21** (nccs-data-bmf #59). Decision as proposed: a critical field fails the report only when more than 1 in 10,000 rows is empty (`CRITICAL_FIELD_MAX_MISSING_SHARE <- 1e-4`); the count is always recorded (`critical_field_missing`), printed and shown in the HTML report; the Z9 emptied-column check is unchanged. Verified on the published June 1996 legacy file: 21 empty of 1,087,620 rows (0.0019%), report passes, so that month no longer needs the gate override. Background: June 1996 legacy has 21 rows with no EIN and the hard gate (Z9) stopped the month for that; the rows carry the EIN-missing flag and cannot join to anything. |
| Z28 | Duplicate parquets in `intermediate/bmf/<vintage>/` double-counted 30 vintages | `nccs-data-bmf` | **PR OPEN 2026-09-21** (nccs-data-bmf #60). Cause: the current pipeline switched to `*_intermediate.parquet` on 2026-01-20 and 30 older `*_processed.parquet` copies (14.9 GB) stayed beside the new files; three DuckDB scripts globbed `*/*.parquet`. Effect, measured by rebuilding: the NTEE-resolved crosswalk published 2026-09-21 morning had inflated `ntee_modal_n` (1.36M rows) and a different `ntee_modal` for 82,945 EINs (it leaned to the recent code); `ntee_current` and `ntee_most_recent` unaffected; address-resolved crosswalk unaffected (counts distinct vintages); Unified BMF unaffected (reads processed CSVs). Fix: globs narrowed to `*_intermediate.parquet`, one-parquet-per-vintage guard; the 30 stale objects DELETED 2026-09-21 (maintainer approved). Pending: republish the corrected crosswalk, release note. |

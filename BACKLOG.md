...
| Z15 | Geocode the address-history spells (new ADR + committee proposal) | `nccs-contracts` then `nccs-data-bmf` | Named-user demand: Lily Johnson (Columbia, doctoral; tracking NY neighborhood orgs year-to-year; told 2026-08-24 we are starting on this — **notify her when it ships**). Add tract/geocode identifiers to the historical address spells in the address-resolved crosswalk. Marginal work is bounded: spell_rank-0 addresses (~3.7M) join back from the geocoded Unified BMF; remaining spells dedupe on normalized (street, city, state, zip5) before hitting the automated geocoder. Design decisions for the ADR: companion table vs new columns (crosswalk is lean join-layer per ADR 0016 — companion likely); `geocode_precision` tier column (pre-2009 legacy spells have no street, geocode to ZIP/city centroid); current-reference-data caveat documented, not solved. On the 2026-09 committee agenda as a §8 prioritization exercise. |
| Z16 | **Fix the NTEEV2 x00 rule in the BMF pipeline (`nteev2fix`)** | `nccs-data-bmf` then `nccs` | **New 2026-08-27.** Our `nteev2` double-encodes specialty/common codes 01-19: `transform_ntee_code.R:354` sets `nteev2_code := ntee_code_clean`, so `B11` renders `EDU-B11-MS` where our own spec (`nccs/_resources/ntee.md:118`) says `EDU-B00-MS` (activity slot x00, type carried by the suffix). ~5.5% of rows. ADR 0032 missed the rule. Found by Jesse (`matchdb` commit `faa4fe8`, `pfmatch/dev/SCHEDI-ALIAS-FINDINGS.md` §6d), patched privately as `bmf_unified_geocoded-nteev2fix.csv`, never reported. Reference semantics: `fiscal::get_clean_ntee()` (3-char with digits <=19 -> x00; 5-char legacy uses positions 4-5). Do: fix + regression test + rebuild lookups/unified/marts/ntee-resolved via the Z13 publisher; amend ADR 0032 with an Outcome note; release note crediting the finding. Then Z2 catalog re-render picks it up. Folds into Z6 (NTEE metadata ADR): the ADR should also state a per-EIN NTEE resolution rule (his: bmf_current / modal / most_recent / bmf_only with an `ntee_rule` column), since `ntee_agreement = mixed` for ~21-31% of EINs. |
| Z17 | Register NODC-published surfaces as contracted producer entries (register, don't ask) | `nccs-contracts` | **New 2026-08-27.** Jesse has write access to `nccsdata` as an external fellow. `npmatch/R/fetch.R:5` hardcodes `nccsdata/crosswalks/npmatch/` (his `MANIFEST.csv`, `raw/normalized/results/` tiers); no contract, ADR, or backlog mention until now. `ef2` publishes 2.8-18 GB DuckDB archives at `nccs-efile/duckdb/efile_v2_2/EFILE<YEAR>.duckdb` (remote ATTACH pattern). Write contract YAMLs describing both as they exist, owner = NODC, accept his manifest format as-is (ADR 0014 convergence noted as future); record in September minutes as "recorded". Also: a BMF column-stability statement in `contracts/unified-bmf.yml` (his `affiliates_bmf()` and `matchdb` hardcode our column names). Decide ownership/terms of `nccs-efile/duckdb/` (NCCS product vs NODC-hosted) before `matchdb` copies the pattern. Context: `~/code/nccs/jesse-nodc-assessment-2026-08-27.md`. Needs SSO to list the prefix. |
| Z18 | Single-EIN lookup helpers in `nccsdata` (+ int32 drift fix in core) | `nccsdata` then `nccs-data-core` | **New 2026-09-03.** ADR 0024 "read helper reused" path, triggered early: the 2026-08 single-org request (`nccs-data-requests/requests/2026-08-single-org-history`) showed one-EIN-since-1989 is the most common non-expert ask and the worst-served path (36+ partition reads; user must know 990pf vs 990combined). Build: `nccs_org_lookup(ein)` (BMF row -> form family via foundation code), `nccs_read_core_ein(ein, years)` (per-year pushdown reads, numeric coercion, stacking), optional plain-language label crosswalk `nccs_core_plain_labels(form)`. Keep lean (arrow+dplyr only; no ProPublica HTTP dep — that stays a request-repo convention). Blocker to not bake in: stacking `merged/990pf` 1989-2024 in one Arrow dataset fails with int32 overflow (`Integer value 3762359200 not in range`) — money columns int32 in some vintages; producer fix in `nccs-data-core` (pin int64/double), tracked in `nccsdata` issue. |

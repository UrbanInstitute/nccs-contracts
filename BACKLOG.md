...
| Z13 | Merge publisher must write `v{YYYY_MM}/` + `latest/` itself | `nccs-data-bmf` | Found at the Z1 re-publish: `merge_master_geocoded_results()` writes `merged/` (old + new names) but never the ADR 0042 vintage prefixes, so `latest/` went stale while the website recommends it as the default link; the 07-26 population of those folders was also manual. Fix in the publisher (idempotent sha256, same as lookups), folding in Z4's default-prefix trap. Until it lands, every rebuild needs a manual copy. |

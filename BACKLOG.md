...
| Z26 | Website "IRS version" NTEE list is out of date | `nccs` | **DONE 2026-09-21** (nccs #101 merged: 24 codes added, P72 kept as retired). Superseded the same day by Z29, which removes the website's own copy and reads the contracted lookup instead. |
| Z29 | Make the `ntee_code` lookup the public NTEE-NAICS crosswalk (long descriptions, NAICS filled, website reads it, dataset page + download) | `nccs-contracts` → `nccs-data-bmf` → `nccs` | **ADR 0049 Accepted 2026-09-21.** Producer DONE: nccs-data-bmf #56 merged; `lookups/bmf/2026_09/` + `latest/` republished 2026-09-21 (`ntee_code.csv` 655 rows, new `ntee_code_description`, NAICS filled for K6A-K6F/K90-K98/N2A/N2B; 15 newer codes have no NODC text, blank). Website: nccs #102 open (dataset page `_datasets/ntee.md`, themed widget reading S3, `data-raw` removed; re-rendered against the live file). Follow-ups: write long descriptions for the 15 blank codes; sibling widgets `ntee1_table` + `ntee_descriptions` still read the NODC file directly. |

...
| G1 | Fix broken geocoding input path + refresh the geocoded master | `nccs-data-bmf` | **Found 2026-07-01.** `run_master_geocoding.R`'s `MASTER_PARQUET_PATH` still hardcodes `data/master/bmf_master.parquet`, but `run_master_pipeline.R` now writes `data/master/bmf_unified.parquet` (stem changed to `UNIFIED_STEM` under ADR 0037) — the next geocoding run will fail until this is repointed. Separately, the published geocoded artifact (`s3://nccsdata/geocoding/bmf-master/merged/`) is stale at 2026-06-17 — predates the 07-01 Unified BMF build, missing `ein_prefixed`/`EIN2` and the newer vintage data. No ADR needed — this is a currency refresh under the existing `bmf-master-geocoded` name/path, not a rename (see G2 for the separate naming question). |

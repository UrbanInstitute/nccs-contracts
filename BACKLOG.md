...
| Z18 | 12 NTEE-CC codes missing from the `ntee_code` lookup sheet (incl. B29 charter schools) | `nccs-data-bmf` | **PR OPEN 2026-09-17** (nccs-data-bmf #54, awaiting maintainer review). Measured on all 121 published months: 5,332 organizations in July 2026 (0.27%), about 511,000 row-observations overall; B29 alone is 2,945 today. All 12 confirmed against the IRS's own list (Form 1023 instructions, Appendix D, rev. 12/2024; identical to Form 1024-A Appendix A). Decision 2026-09-17: the IRS appendix is the source of truth for the code list, not a third-party copy. The PR adds the 12 rows with IRS wording, commits the IRS list as `data/lookup/irs_ntee_codes.csv`, adds a refresh script and runbook, and empties the pinned test list. Nine codes the IRS has retired stay in the sheet. **After merge:** reprocess all 121 months, rebuild Unified BMF + NTEE-resolved crosswalk; then Z25 and Z26. |
...
| Z26 | Website "IRS version" NTEE list is out of date | `nccs` | **New 2026-09-17** (from Z18). `data-raw/NTEE-NAICS-XWALK.csv` (feeds the NTEE resources page) lacks the seven codes the IRS added in 2021 and still lists P72, which the IRS has retired but 278 organizations in the July 2026 file still carry. Refresh from `data/lookup/irs_ntee_codes.csv` in nccs-data-bmf (645 codes), keep NAICS from the existing crosswalk, and decide P72: keep as a retired code with a note, or drop. Do after Z18's reprocess so the site and the data agree. |
...
| Z28 | Stale duplicate parquets under `intermediate/bmf/<vintage>/` inflate any glob-based count | `nccs-data-bmf` | **New 2026-09-17** (found while verifying Z18). Most current-month folders hold two files: an older `bmf_YYYY_MM_processed.parquet` (written 2026-01-21/22) and the pipeline's `bmf_YYYY_MM_intermediate.parquet`. `scripts/measure_ntee_lookup_gaps.R` and `scripts/build_ntee_resolved_crosswalk.R` both glob `*.parquet` per folder, so those months count twice (the Z18 measurement said ~511k observations; the true rows-changed figure from the reprocess is 379,258). Check whether the crosswalk's per-EIN vintage counts (`n_vintages_with_ntee`) are inflated, delete the stale files (or make the globs pick one file per folder), and rerun the measurement. |

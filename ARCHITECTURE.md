...
| Component | Role | Repo |
|---|---|---|
| BMF pipeline | Producer — the **Unified BMF** (un-geocoded rolling master, renamed from "master" + non-silently superseding `master/bmf/` → `unified/bmf/`, per ADR 0037; carries additive `ein_prefixed`/`EIN2` per ADR 0036) + its geocoded extension (itself renamed from `bmf-master-geocoded`, extending the rename one layer deeper + moving state marts under `unified/bmf/`, per ADR 0039) + dated vintages + lookups + geography crosswalks (`county-fips`, `cbsa`, and the coordinate-keyed `ct-planning-region` under `s3://nccsdata/crosswalks/`, per ADR 0021 + ADR 0023) + the per-EIN cross-vintage NTEE-recovery crosswalk (`ntee-resolved`, per ADR 0034). Geocoding runs through Urban's **automated tech-tools geocoder service** (S3-event-driven: batch CSVs with a single-line `f_address` dropped in `s3://geocoding-codestar-prod/data/input-data/` auto-start the ArcGIS engine via Lambda, results appear under `data/output-data/`, the engine self-stops; infra `UI-Research/techforms-geocoding`; no manual activation — see `nccs-data-bmf/docs/reference/geocoder-service.md`) | `nccs-data-bmf` |
| Core 990 pipeline | Producer — IRS SOI extracts, one row per filing. Dual-published as parquet (service-tier canonical, the API's read target) + a CSV mirror on a 90-day window, per ADR 0027; multi-year parquet reads use `union_by_name` (cross-vintage type drift; producer type-stabilization is a fast-follow). | `nccs-data-core` |
| E-file pipeline | Producer — LIVE (Phase 0). Parses raw IRS 990/990-PF XML into form-agnostic parquet; first vintage `v2026.05` published 2026-05-29. Phase 0 ships two filing-grain metric tables (`government_grants`, `program_related_investments_total`) under `s3://nccsdata/processed/efile/phase0/`, grandfathered as the first curated view under ADR 0028. ADR 0007's Phase 1+ curated-extraction roadmap is **superseded by ADR 0028** (wholesale extraction to a normalized relational tier, uncontracted researcher catalog + contracted curated views) — architecture ADR + extractor/publish path built (`nccs-data-efile` PRs #13/#15/#16), but **not yet run/published** as of 2026-07-01. See ADR 0007 / ADR 0017 / ADR 0028. | `nccs-data-efile` |
...
| API | Consumer + service tier — `sector-in-brief-api`; DuckDB-on-parquet with query-time EIN joins (no pre-merged artifact, per ADR 0016). Two query modes: CORE-join (filing-level, filtered by `tax_years`) and `source=bmf` (org-level registry read of the geocoded Unified BMF, formerly `bmf-master-geocoded`, renamed by ADR 0039, incl. non-filers, filtered by `active_years` lifespan overlap, per ADR 0029). Lambda-first hybrid host with an async worker for the p99+ giant tail (ADR 0008 Outcome, Phase-0 2026-06-09); pattern-B downloads via durable `/download/{job_id}` (ADR 0026); per-form CORE tier routing, API-canonical (ADR 0031). **Live in prod** (slices 1–5.1) — staging 2026-06-09, prod deploy + sector-in-brief UI cutover 2026-06-12; 1-week soak underway, legacy-API sunset pending. | `sector-in-brief-api` |

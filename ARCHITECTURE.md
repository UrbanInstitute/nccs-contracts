...
| Component | Role | Repo |
|---|---|---|
| BMF pipeline | Producer — geocoded master + dated vintages + lookups + geography crosswalks (`county-fips`, `cbsa`, and the coordinate-keyed `ct-planning-region` under `s3://nccsdata/crosswalks/`, per ADR 0021 + ADR 0023) + the per-EIN cross-vintage NTEE-recovery crosswalk (`ntee-resolved`, per ADR 0034) | `nccs-data-bmf` |
